...
const KnowledgeTypingGame = ({ word, userId, courseName, onComplete, examplePair = null }) => {
  ...
  const expected = (examplePair?.target || word);
  ...
  useEffect(() => {
    const load = async () => {
      ...
      // ✅ Если пришёл AI пример + перевод — используем и не дергаем сеть
      ...
      // 🔁 Fallback: старое поведение (как было)
      let example = "";
      try {
...
const getMaskedSentence = () => {
  const index = sentence.toLowerCase().indexOf(expected.toLowerCase());
  if (index === -1) return sentence;

  const before = sentence.slice(0, index);
  const after = sentence.slice(index + expected.length);
...
if (val.trim().toLowerCase() === expected.toLowerCase() && !completed) {
      setCompleted(true);
      try {
...
disabled={completed || revealed >= expected.length}      >
        Показать букву
      </button>
    </div>
  );
